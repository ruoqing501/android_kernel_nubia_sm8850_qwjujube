		...
		void *display, u32 bl_lvl)
{
...
#ifdef CONFIG_DRM_ZTE_DISP_LTM
    // add for sensor and ltm both adjust backlight.SENDOR_BL_UP=1, SENDOR_BL_DOWN=2, SENDOR_BL_EXIT=0
    if (bl_scale_sv > MAX_SV_BL_SCALE_LEVEL && bl_temp > 0 && last_ltm_bl_temp > 0) {
        //DSI_INFO("msm_lcd last_bl_temp=%u to last_ltm_bl_temp=%llu, ltm_sensor=%d, bl_scale_sv=%u, bl_lvl=%u to ltm bl_temp=%u\n",
		//last_bl_temp, last_ltm_bl_temp, panel->disp_feature[ZTE_LCD_LTM_SENSOR_BL].mode, bl_scale_sv, bl_lvl, (u32)bl_temp);

        if (panel->disp_feature[ZTE_LCD_LTM_SENSOR_BL].mode == 1 && bl_temp <= last_ltm_bl_temp && \
            last_bl_temp <= bl_lvl) {
            //bl_temp = last_ltm_bl_temp;
            goto error;
        } else if (panel->disp_feature[ZTE_LCD_LTM_SENSOR_BL].mode == 2 && bl_temp >= last_ltm_bl_temp && \
            last_bl_temp >= bl_lvl) {
            //bl_temp = last_ltm_bl_temp;
            goto error;
        }
    }
	// add for ltm bl_level expend the normal max backlight
    if (bl_scale_sv > MAX_SV_BL_SCALE_LEVEL && panel->bl_normalmax_level > 0 \
		&& panel->bl_config.bl_level <= panel->bl_normalmax_level) {
		if (bl_temp > panel->bl_normalmax_level)
			bl_temp = panel->bl_normalmax_level;
    }
	if (bl_lvl == 0 && bl_scale_sv != MAX_SV_BL_SCALE_LEVEL) {
		DSI_INFO("msm_lcd LTM bl_scale_sv = %u sleep\n", bl_scale_sv);
	}
#endif
	...
	DSI_DEBUG("msm_lcd bl_scale = %u, bl_scale_sv = %u, bl_lvl = %u\n",
...
static bool dsi_display_validate_reg_read(struct dsi_panel *panel)
{
	...
	if (panel->disp_feature[ZTE_LCD_TEMP_DEBUG].mode == ZTE_SKIP_ESD) { // 1
		DRM_ERROR("msm_lcd mismatch: skip esdcheck\n");
		return true;
	}
#endif
		...
		int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_LP1:
		...
		#ifdef CONFIG_DRM_ZTE_DISP
		zte_aod_handler(display->panel, PM_AOD_ON);
		display->panel->skip_nolp = false;
		#endif
		break;
	...
	case SDE_MODE_DPMS_ON:
		...
		if ((display->panel->power_mode == SDE_MODE_DPMS_LP1) ||
			(display->panel->power_mode == SDE_MODE_DPMS_LP2)) {
				if (!display->panel->skip_nolp) {
					rc = dsi_panel_set_nolp(display->panel);
					zte_aod_handler(display->panel, PM_AOD_OFF);
					display->panel->has_bl = false;
				}
		}
		#else
		if ((display->panel->power_mode == SDE_MODE_DPMS_LP1) ||

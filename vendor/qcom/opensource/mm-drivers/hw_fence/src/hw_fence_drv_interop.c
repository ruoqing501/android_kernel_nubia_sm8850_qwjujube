...
int hw_fence_interop_share_handle_status(struct synx_import_indv_params *params, u64 h_synx,
	u32 *signal_status)
{
...
void *hw_fence_interop_get_fence(u32 h_synx)
{
	...
	if (!(hw_fence_is_valid_hw_fence_handle(hw_fence_drv_data, h_synx))) {
		HWFNC_ERR("invalid h_synx:%u handle bit:%lu drv_id:%d\n",
			h_synx, (unsigned long)SYNX_HW_FENCE_HANDLE_FLAG, hw_fence_drv_data->drv_id);

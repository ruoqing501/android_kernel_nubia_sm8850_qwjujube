__int64 __fastcall gen8_hwsched_handle_watchdog(__int64 a1)
{
  __int64 result; // x0
  int v3; // [xsp+4h] [xbp-Ch] BYREF
  __int64 v4; // [xsp+8h] [xbp-8h]

  v4 = *(_QWORD *)(_ReadStatusReg(SP_EL0) + 1808);
  v3 = 0;
  gmu_core_regread(a1, 146182, &v3);
  gmu_core_regwrite(a1, 146182, v3 | 1u);
  gen8_gmu_send_nmi(a1, 0, 0);
  if ( (unsigned int)__ratelimit(&gen8_hwsched_handle_watchdog__rs, "gen8_hwsched_handle_watchdog") )
    dev_err(*(_QWORD *)(a1 + 1544) + 16LL, "GMU watchdog expired interrupt received\n");
  result = gen8_hwsched_fault(a1, 32);
  _ReadStatusReg(SP_EL0);
  return result;
}

unsigned __int64 __fastcall gen8_gmu_first_open(unsigned __int64 a1)
{
  ...
  _X20 = (unsigned __int64 *)(a1 - 784);
  if ( (*(_QWORD *)(a1 - 784) & 1) != 0 )
  {
    if ( (*_X20 & 2) != 0 || (result = gen8_boot(a1), !(_DWORD)result) )
    {
LABEL_21:
  ...
  {
    result = ((__int64 (__fastcall *)(unsigned __int64))gen8_ringbuffer_init)(a1);
    if ( !(_DWORD)result )
    {
      ...
      if ( !(_DWORD)result )
      {
        ...
        if ( !(_DWORD)result )
        {
          result = ((__int64 (__fastcall *)(unsigned __int64))gen8_gmu_parse_fw)(a1);
          if ( !(_DWORD)result )
          {
            ...
            if ( !result || result >= 0xFFFFFFFFFFFFF001LL )
            {
              result = ((__int64 (__fastcall *)(unsigned __int64, _QWORD, __int64, __int64, _QWORD))gmu_core_reserve_kernel_block)(
                         ...
                         4,
                         0);
              *(_QWORD *)(a1 - 1808) = result;
            }
            if ( result < 0xFFFFFFFFFFFFF001LL )
            {
              result = gen8_hfi_init(a1);
              if ( !(_DWORD)result )
              {
                kgsl_pwrctrl_request_state(a1, 2);
                result = gen8_gmu_first_boot(a1);
                if ( !(_DWORD)result )
                {
                  gen8_populate_ctxt_record_size(a1);
                  gen8_preemption_init(a1);

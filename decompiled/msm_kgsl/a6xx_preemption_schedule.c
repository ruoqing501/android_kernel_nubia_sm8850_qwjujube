__int64 __fastcall a6xx_preemption_schedule(__int64 result)
{
  __int64 v1; // x19
  __int64 v2; // x20
  int v3; // w8
  int v4; // w0
  int *v5; // x8
  int v6; // w22
  const char *v7; // x0
  __int64 v8; // x8
  __int64 v9; // x9
  __int64 v10; // x20
  int v11; // w22
  int v12; // w21
  int rptr; // w25
  int v14; // w23
  int v15; // w24
  int v16; // w0
  unsigned int v23; // w9

  if ( (*(_QWORD *)(result + 14240) & 0x200) != 0 )
  {
    v1 = result;
    v2 = result + 20480;
    rt_mutex_lock(result + 11088);
    if ( *(_DWORD *)(v2 + 8) == 5 )
    {
      v3 = *(_DWORD *)(v1 + 11120);
      if ( v3 == 32 || v3 == 2 )
      {
        v4 = kgsl_regmap_read(v1 + 13200);
        if ( (v4 & 1) != 0 )
        {
          v10 = *(_QWORD *)v1;
          v11 = v4;
          v12 = *(_DWORD *)(*(_QWORD *)(v1 + 19640) + 28LL);
          rptr = adreno_get_rptr(*(_QWORD *)(v1 + 19640));
          v14 = *(_DWORD *)(*(_QWORD *)(v1 + 19640) + 20LL);
          v15 = *(_DWORD *)(*(_QWORD *)(v1 + 19648) + 28LL);
          v16 = adreno_get_rptr(*(_QWORD *)(v1 + 19648));
          dev_err(
            v10,
            "Preemption not complete: status=%X cur=%d R/W=%X/%X next=%d R/W=%X/%X\n",
            v11,
            v12,
            rptr,
            v14,
            v15,
            v16,
            *(_DWORD *)(*(_QWORD *)(v1 + 19648) + 20LL));
          _X8 = (unsigned int *)(v1 + 24448);
          __asm { PRFM            #0x11, [X8] }
          do
            v23 = __ldxr(_X8);
          while ( __stxr(v23 | 0x10, _X8) );
          __dmb(0xAu);
          kthread_queue_work(*(_QWORD *)(v1 + 24400), v1 + 24408);
        }
        else
        {
          ++*(_DWORD *)(v2 + 104);
          if ( (gmu_core_isenabled(v1) & 1) != 0 )
          {
            v5 = *(int **)(v1 + 14264);
            v6 = *v5;
            if ( *v5 == 660
              || (v7 = *((const char **)v5 + 3)) != nullptr && !strcmp(v7, "qcom,adreno-gpu-a642l")
              || (unsigned int)(v6 - 643) <= 0x14 && ((1 << (v6 + 125)) & 0x180001) != 0 )
            {
              gmu_core_regwrite(v1, 129220, 0);
            }
            else
            {
              gmu_core_regrmw(v1, 146198, 2, 0);
            }
          }
          timer_delete_sync(v1 + 20504);
          kgsl_regmap_read(v1 + 13200);
          v8 = *(_QWORD *)(v1 + 19640);
          v9 = *(_QWORD *)(v1 + 19648);
          *(_QWORD *)(v1 + 19648) = 0;
          *(_QWORD *)(v1 + 19656) = v8;
          *(_QWORD *)(v1 + 19640) = v9;
          update_wptr((_QWORD *)v1, 1, 0);
          mod_timer(v1 + 19816);
          __dmb(0xAu);
          *(_DWORD *)(v2 + 8) = 0;
          __dmb(0xAu);
        }
      }
    }
    a6xx_preemption_trigger(v1, 0);
    return rt_mutex_unlock(v1 + 11088);
  }
  return result;
}

__int64 do_gpu_maxfreq_step_up()
{
  __int64 (*v0)(void); // x8
  int v1; // w0
  __int64 (*v2)(void); // x8
...
LABEL_17:
  ...
  mutex_lock(&gpu_ctrl_lock);
  if ( v6 > gpu_loading_low )
  {
    if ( !(cur_gpu_max_level | gpu_max_pwrlevel_ceiling) )
    {
      if ( ifas_log_enable )
        printk(&unk_F56A, "do_gpu_maxfreq_step_up");
      return mutex_unlock(&gpu_ctrl_lock);
    }
    v8 = (cur_gpu_max_level - (rescue_gpu_level_up + 1)) & ~((cur_gpu_max_level - (rescue_gpu_level_up + 1)) >> 31);
    if ( cur_gpu_max_level < rescue_gpu_level_up + 1 )
      v8 = 0;
    cur_gpu_max_level = v8;
    if ( ifas_log_enable )
      printk(&unk_FF28, "do_gpu_maxfreq_step_up");
  }

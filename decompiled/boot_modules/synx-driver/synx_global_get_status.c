__int64 __fastcall synx_global_get_status(unsigned int a1, __int64 a2, __int64 a3, __int64 a4, __int64 a5, __int64 a6)
{
  __int64 v7; // x4
  __int64 v8; // x5
  __int64 result; // x0
  __int64 v10; // x8
  __int64 v11; // x9
  unsigned int v12; // w21
  unsigned int v13; // w20
  __int64 v14; // x5
  __int64 v15; // x8
  __int64 v16; // x0
  _QWORD v17[2]; // [xsp+0h] [xbp-10h] BYREF

  v17[1] = *(_QWORD *)(_ReadStatusReg(SP_EL0) + 1808);
  v17[0] = 0;
  if ( !synx_gmem_2 )
  {
    if ( (synx_debug & 1) != 0 )
    {
      printk(&unk_267BF, &unk_29207, "synx_global_get_status", 637, a5, a6);
      result = 0;
      goto LABEL_30;
    }
LABEL_9:
    result = 0;
    goto LABEL_30;
  }
  if ( a1 - 1 >= 0xFFF )
  {
    if ( (synx_debug & 1) != 0 )
    {
      printk(&unk_27C7E, &unk_29207, "synx_global_get_status", 642, a1, a6);
      result = 0;
      goto LABEL_30;
    }
    goto LABEL_9;
  }
  if ( (get_ipclite_feature(4) & 1) != 0 )
  {
    result = ipclite_global_spin_lock_timeout((unsigned __int8)a1, 500, v17);
    if ( !(_DWORD)result )
    {
      v10 = synx_gmem_2;
LABEL_13:
      v11 = v10 + glcoredata_size * a1;
      if ( *(_DWORD *)v11 == 1 )
      {
        v12 = 1;
      }
      else if ( *(_WORD *)(v11 + 10) )
      {
        v12 = 1;
      }
      else
      {
        v12 = *(_DWORD *)v11;
      }
      if ( (get_ipclite_feature(4) & 1) != 0 )
      {
        ipclite_global_spin_unlock((unsigned __int8)a1, v17);
      }
      else
      {
        v15 = synx_gmem_2;
        if ( *(_WORD *)(synx_gmem_2 + 12) && (synx_debug & 2) != 0 )
        {
          printk(&unk_29DF8, "warn", "synx_gmem_lock_owner_clear", 42, *(unsigned __int16 *)(synx_gmem_2 + 12), v14);
          v15 = synx_gmem_2;
        }
        v16 = synx_hwlock;
        *(_WORD *)(v15 + 12) = 8;
        _hwspin_unlock(v16, 1, v17);
      }
      result = v12;
      goto LABEL_30;
    }
  }
  else if ( synx_hwlock )
  {
    result = _hwspin_lock_timeout(synx_hwlock, 500, 1, v17);
    if ( !(_DWORD)result )
    {
      v10 = synx_gmem_2;
      *(_WORD *)(synx_gmem_2 + 12) = 0;
      goto LABEL_13;
    }
  }
  else
  {
    if ( (synx_debug & 1) != 0 )
      printk(&unk_2AE54, &unk_29207, "synx_gmem_lock", 54, v7, v8);
    result = 4294967274LL;
  }
  if ( (synx_debug & 1) != 0 )
  {
    v13 = result;
    printk(&unk_2981B, &unk_29207, "synx_global_get_status", 648, a1, v8);
    result = v13;
  }
LABEL_30:
  _ReadStatusReg(SP_EL0);
  return result;
}

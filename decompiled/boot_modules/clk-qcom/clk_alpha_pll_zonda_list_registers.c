__int64 __fastcall clk_alpha_pll_zonda_list_registers(__int64 a1, __int64 a2)
{
  __int64 v4; // x23
  __int64 result; // x0
  ...
  do
  {
    regmap_read(
      *(_QWORD *)(a2 + 24),
      *(_DWORD *)(a2 - 48)
    + (unsigned int)*(unsigned __int8 *)(*(_QWORD *)(a2 - 40)
                                       + LODWORD((&clk_alpha_pll_zonda_list_registers_data)[v4 + 1])),
      &v6);
    if ( a1 )
      seq_printf(a1, "%20s: 0x%.8x\n", (&clk_alpha_pll_zonda_list_registers_data)[v4], v6);
    else
      printk(&unk_28721);
    v4 += 2;
  }

__int64 __fastcall perf_trace_rdev_set_qos_map(__int64 result, __int64 a2, __int64 a3, __int64 a4)
{
  __int64 v4; // x30
  __int64 v5; // x24
  __int64 v8; // x19
  __int64 v10; // x25
  unsigned __int64 StatusReg; // x26
  _QWORD *v12; // x9
  __int64 v13; // x21
  const char *v14; // x24
  size_t v15; // x0
  unsigned __int64 v16; // x2
  __int64 v17; // x8
  __int64 v18; // x9
  __int64 v19; // x9
  __int64 v20; // x10
  __int64 v21; // x11
  __int64 v22; // x9
  long double v23; // q0
  __int64 v24; // x7
  __int64 v25; // [xsp+0h] [xbp-20h] BYREF
  unsigned int v26; // [xsp+Ch] [xbp-14h] BYREF
  _QWORD v27[2]; // [xsp+10h] [xbp-10h] BYREF
  __int64 vars0; // [xsp+20h] [xbp+0h] BYREF

  v5 = v4;
  v8 = result;
  v27[1] = *(_QWORD *)(_ReadStatusReg(SP_EL0) + 1808);
  v27[0] = 0;
  v10 = *(_QWORD *)(result + 104);
  v26 = 0;
  StatusReg = _ReadStatusReg(TPIDR_EL1);
  if ( !*(_QWORD *)(result + 112) && !*(_QWORD *)(StatusReg + v10) )
    goto LABEL_15;
  result = perf_trace_buf_alloc(124, v27, &v26);
  if ( !result )
    goto LABEL_15;
  v12 = (_QWORD *)v27[0];
  v13 = result;
  *(_QWORD *)(v27[0] + 232LL) = &vars0;
  v12[33] = 5;
  v12[31] = &v25;
  v12[32] = v5;
  v14 = *(const char **)(a2 + 504);
  if ( !v14 )
    v14 = *(const char **)(a2 + 392);
  v15 = strnlen(v14, 0x20u);
  if ( v15 == -1 )
  {
    _fortify_panic(2, -1);
  }
  else
  {
    if ( v15 == 32 )
      v16 = 32;
    else
      v16 = v15 + 1;
    if ( v16 < 0x21 )
    {
      sized_strscpy(v13 + 8, v14);
      v17 = *(_QWORD *)(a3 + 304);
      *(_QWORD *)(v13 + 40) = *(_QWORD *)(a3 + 296);
      *(_QWORD *)(v13 + 48) = v17;
      *(_DWORD *)(v13 + 56) = *(_DWORD *)(a3 + 224);
      if ( a4 )
      {
        *(_BYTE *)(v13 + 60) = *(_BYTE *)a4;
        v18 = *(_QWORD *)(a4 + 1);
        *(_QWORD *)(v13 + 69) = *(_QWORD *)(a4 + 9);
        *(_QWORD *)(v13 + 61) = v18;
        v19 = *(_QWORD *)(a4 + 33);
        v20 = *(_QWORD *)(a4 + 17);
        v21 = *(_QWORD *)(a4 + 25);
        *(_WORD *)(v13 + 101) = *(_WORD *)(a4 + 41);
        *(_QWORD *)(v13 + 93) = v19;
        *(_QWORD *)(v13 + 85) = v21;
        *(_QWORD *)(v13 + 77) = v20;
        v22 = *(_QWORD *)(a4 + 43);
        *(_QWORD *)(v13 + 111) = *(_QWORD *)(a4 + 51);
        *(_QWORD *)(v13 + 103) = v22;
      }
      else
      {
        *(_BYTE *)(v13 + 60) = 0;
        *(_QWORD *)(v13 + 61) = 0;
        *(_QWORD *)(v13 + 69) = 0;
        *(_QWORD *)(v13 + 77) = 0;
        *(_QWORD *)(v13 + 85) = 0;
        *(_QWORD *)(v13 + 93) = 0;
        *(_WORD *)(v13 + 101) = 0;
        *(_QWORD *)(v13 + 103) = 0;
        *(_QWORD *)(v13 + 111) = 0;
      }
      result = perf_trace_run_bpf_submit(v13, 124, v26, v8, 1, v27[0], StatusReg + v10, 0);
LABEL_15:
      _ReadStatusReg(SP_EL0);
      return result;
    }
  }
  v23 = _fortify_panic(7, 32);
  if ( v24 )
    JUMPOUT(0x1401CC);
  return trace_event_raw_event_rdev_set_ap_chanwidth(v23);
}

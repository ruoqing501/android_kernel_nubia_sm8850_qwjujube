__int64 __fastcall perf_trace_cfg80211_control_port_tx_status(__int64 result, unsigned __int64 a2, __int64 a3, char a4)
{
  __int64 v4; // x30
  __int64 v5; // x23
  __int64 v8; // x19
  __int64 v10; // x24
  unsigned __int64 StatusReg; // x25
  _QWORD *v12; // x10
  int v13; // w8
  __int64 v14; // x2
  __int64 v15; // x5
  __int64 v16; // [xsp+0h] [xbp-20h] BYREF
  unsigned int v17; // [xsp+Ch] [xbp-14h] BYREF
  _QWORD v18[2]; // [xsp+10h] [xbp-10h] BYREF
  __int64 v19; // [xsp+20h] [xbp+0h] BYREF

  v5 = v4;
  v8 = result;
  v18[1] = *(_QWORD *)(_ReadStatusReg(SP_EL0) + 1808);
  v18[0] = 0;
  v10 = *(_QWORD *)(result + 104);
  v17 = 0;
  StatusReg = _ReadStatusReg(TPIDR_EL1);
  if ( *(_QWORD *)(result + 112) || *(_QWORD *)(StatusReg + v10) )
  {
    result = perf_trace_buf_alloc(36, v18, &v17);
    if ( result )
    {
      v12 = (_QWORD *)v18[0];
      v13 = 0;
      *(_QWORD *)(v18[0] + 232LL) = &v19;
      v12[33] = 5;
      v12[31] = &v16;
      v12[32] = v5;
      if ( a2 && a2 <= 0xFFFFFFFFFFFFF000LL )
        v13 = *(_DWORD *)(a2 + 40);
      v14 = v17;
      v15 = v18[0];
      *(_DWORD *)(result + 8) = v13;
      *(_QWORD *)(result + 16) = a3;
      *(_BYTE *)(result + 24) = a4 & 1;
      result = perf_trace_run_bpf_submit(result, 36, v14, v8, 1, v15, StatusReg + v10, 0);
    }
  }
  _ReadStatusReg(SP_EL0);
  return result;
}

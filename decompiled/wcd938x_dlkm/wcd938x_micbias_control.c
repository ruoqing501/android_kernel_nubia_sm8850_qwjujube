__int64 __fastcall wcd938x_micbias_control(__int64 a1, int a2, int a3, int a4)
{
  __int64 v4; // x21
  __int64 v7; // x26
  int v8; // w27
  ...
  {
    if ( (unsigned int)__ratelimit(&wcd938x_micbias_control__rs, "wcd938x_micbias_control") )
    {
      dev_err(
        *(_QWORD *)(a1 + 24),
        "%s: Invalid micbias index, micb_ind:%d\n",
        "wcd938x_micbias_control",
        (unsigned int)v4);
      return 4294967274LL;
    }
    return 4294967274LL;
  ...
  if ( !v7 )
  {
    if ( (unsigned int)__ratelimit(&wcd938x_micbias_control__rs_6, "wcd938x_micbias_control") )
    {
      dev_err(*(_QWORD *)(a1 + 24), "%s: wcd938x private data is NULL\n", "wcd938x_micbias_control");
      return 4294967274LL;
    }
    return 4294967274LL;
  }
...
LABEL_15:
      if ( (unsigned int)__ratelimit(&wcd938x_micbias_control__rs_8, "wcd938x_micbias_control") )
      {
        dev_err(*(_QWORD *)(a1 + 24), "%s: Invalid micbias number: %d\n", "wcd938x_micbias_control", (unsigned int)a2);
        return 4294967274LL;
      }
      return 4294967274LL;
    }
    ...
    if ( v8 == 2 )
    {
      if ( *(_BYTE *)(v7 + 3192) == 1 )
      {
        ...
        *(_DWORD *)(v28 + 56) = v30 + 1;
        if ( !v30 )
        {
          snd_soc_component_update_bits(a1, 13321, 240, 240);
          snd_soc_component_update_bits(a1, 13320, 16, 16);
          snd_soc_component_update_bits(a1, 13335, 1, 1);
          snd_soc_component_update_bits(a1, 12396, 1, 1);
          snd_soc_component_update_bits(a1, 12399, 1, 1);
          snd_soc_component_update_bits(a1, 12402, 1, 1);
          snd_soc_component_update_bits(a1, 12405, 1, 1);

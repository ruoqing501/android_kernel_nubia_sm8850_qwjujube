__int64 __fastcall sde_cp_ltm_hist_interrupt(_QWORD *a1, char a2, _QWORD *a3)
{
  __int64 v4; // x9
  _QWORD *v5; // x19
  __int64 v6; // x2
...
LABEL_10:
  ...
  if ( v6 )
  {
    if ( (a2 & 1) != 0 )
    {
      result = sde_cp_ltm_register_irq(*(_QWORD *)(v4 + 8), a1);
      if ( (_DWORD)result )
      {
        v8 = result;
        _drm_err("failed to register stats_done irq\n");
        result = v8;
      }
    }
    else
    {
      v9 = sde_core_irq_idx_lookup(*(_QWORD *)(v4 + 8), 35, *(unsigned int *)(v6 + 64));
      v17 = v9;
      if ( (v9 & 0x80000000) != 0 )
      {
        v16 = v9;
      ...
      _drm_err("failed to unregister stats_done irq\n");
      result = v16;
    }
  }
  else

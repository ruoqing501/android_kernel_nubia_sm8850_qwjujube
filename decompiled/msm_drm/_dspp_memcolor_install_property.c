__int64 __fastcall dspp_memcolor_install_property(__int64 *a1)
{
  __int64 v1; // x8
  __int64 v2; // x8
  int v3; // w1
  ...
  *(_QWORD *)s = 0;
  v7 = 0;
  v3 = HIWORD(*(_DWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(v2 + 8) + 152LL) + 8216LL) + 176LL));
  if ( v3 == 1 )
  {
    snprintf(s, 0x100u, "%s%d", "SDE_DSPP_PA_MEMCOL_SKIN_V", 1);
    sde_cp_crtc_install_blob_property(a1, s, 4, 48);
    snprintf(s, 0x100u, "%s%d", "SDE_DSPP_PA_MEMCOL_SKY_V", 1);
    sde_cp_crtc_install_blob_property(a1, s, 5, 48);
    snprintf(s, 0x100u, "%s%d", "SDE_DSPP_PA_MEMCOL_FOLIAGE_V", 1);
    sde_cp_crtc_install_blob_property(a1, s, 6, 48);
    snprintf(s, 0x100u, "%s%d", "SDE_DSPP_PA_MEMCOL_PROT_V", 1);
    result = sde_cp_crtc_install_blob_property(a1, s, 7, 48);
  }
  else
  {

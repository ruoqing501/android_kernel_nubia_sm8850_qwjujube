__int64 __fastcall cnss_event_work_init(__int64 a1)
{
  __int64 v2; // x0
  __int64 v3; // x6
  __int64 v4; // x7
  unsigned __int64 StatusReg; // x8
  const char *v7; // x1
  __int16 v8; // w9
  const char *v9; // x8
  char v10; // [xsp+0h] [xbp+0h]

  *(_DWORD *)(a1 + 640) = 0;
  v2 = alloc_workqueue("cnss_driver_event", 2, 1);
  *(_QWORD *)(a1 + 680) = v2;
  if ( v2 )
  {
    *(_QWORD *)(a1 + 648) = 0xFFFFFFFE00000LL;
    *(_QWORD *)(a1 + 656) = a1 + 656;
    *(_QWORD *)(a1 + 664) = a1 + 656;
    *(_QWORD *)(a1 + 672) = cnss_driver_event_work;
    *(_QWORD *)(a1 + 560) = a1 + 560;
    *(_QWORD *)(a1 + 568) = a1 + 560;
    return 0;
  }
  else
  {
    StatusReg = _ReadStatusReg(SP_EL0);
    if ( (*(_DWORD *)(StatusReg + 16) & 0xF0000) != 0 )
    {
      v7 = "irq";
    }
    else
    {
      v8 = *(_DWORD *)(StatusReg + 16);
      v9 = (const char *)(StatusReg + 2320);
      if ( (v8 & 0xFF00) != 0 )
        v7 = "soft_irq";
      else
        v7 = v9;
    }
    cnss_debug_ipc_log_print(
      cnss_ipc_log_context,
      v7,
      "cnss_event_work_init",
      3u,
      3u,
      "Failed to create event workqueue!\n",
      v3,
      v4,
      v10);
    return 4294967282LL;
  }
}

__int64 __fastcall ipa_rm_delete_dependency_from_ioctl(unsigned int a1, unsigned int a2)
{
...
LABEL_8:
        ipc_log_string(ipc_logbuf, "ipa_rm %s:%d %s -> %s\n", "_ipa_rm_delete_dependency", 189, v5, v6);
LABEL_9:
        if ( !ipa3_get_ipc_logbuf_low() )
        {
LABEL_16:
          v10 = raw_spin_lock_irqsave(ipa_rm_ctx + 16);
          v11 = ((__int64 (__fastcall *)(_QWORD, _QWORD, _QWORD, __int64))ipa_rm_dep_graph_delete_dependency)(
                  *(_QWORD *)ipa_rm_ctx,
                  a1,
                  a2,
                  1);
...
LABEL_15:
            ipc_log_string(v7, "ipa_rm %s:%d %s -> %s\n", "_ipa_rm_delete_dependency", 189, v8, v9);
            goto LABEL_16;
          }
        }
        v9 = "INVALID RESOURCE";

__int64 __fastcall ipa3_cfg_ep_hdr_ext(unsigned int a1, _DWORD *a2)
{
  __int64 v3; // x8
  unsigned int v4; // w22
  __int64 v6; // x0
...
LABEL_61:
  ...
  if ( a2 && *(_DWORD *)(ipa3_ctx + 480LL * a1 + 168) )
  {
    if ( ipa3_ctx )
    {
      ...
      if ( v6 )
      {
        ipc_log_string(v6, "ipa %s:%d pipe=%d hdr_pad_to_alignment=%d\n", "ipa3_cfg_ep_hdr_ext", 9246, a1, *a2);
        v3 = ipa3_ctx;
      }
      v7 = *(_QWORD *)(v3 + 34160);
      if ( v7 )
      {
        ipc_log_string(v7, "ipa %s:%d pipe=%d hdr_pad_to_alignment=%d\n", "ipa3_cfg_ep_hdr_ext", 9246, a1, *a2);
        v3 = ipa3_ctx;
      }
      if ( v3 )
      {
        ...
        if ( v8 )
        {
          ipc_log_string(v8, "ipa %s:%d hdr_total_len_or_pad_offset=%d\n", "ipa3_cfg_ep_hdr_ext", 9249, a2[1]);
          v3 = ipa3_ctx;
        }
        v9 = *(_QWORD *)(v3 + 34160);
        if ( v9 )
        {
          ipc_log_string(v9, "ipa %s:%d hdr_total_len_or_pad_offset=%d\n", "ipa3_cfg_ep_hdr_ext", 9249, a2[1]);
          v3 = ipa3_ctx;
        }
        if ( v3 )
        {
          ...
          if ( v10 )
          {
            ipc_log_string(
              v10,
              "ipa %s:%d hdr_payload_len_inc_padding=%d hdr_total_len_or_pad=%d\n",
              "ipa3_cfg_ep_hdr_ext",
              9253,
          ...
          if ( v3 )
          {
            ...
            if ( v12 )
            {
              ipc_log_string(
                v12,
                "ipa %s:%d hdr_total_len_or_pad_valid=%d hdr_little_endian=%d\n",
                "ipa3_cfg_ep_hdr_ext",
                9257,
    ...
    ((void (__fastcall *)(__int64, _QWORD, _QWORD *))ipahal_write_reg_n_fields)(42, a1, v16 + 45);
    if ( *(_DWORD *)(ipa3_ctx + 34308) <= a1 )
    {
      printk(&unk_3E7136, "ipa3_get_client_mapping");
      v29 = ipa3_ctx;
...
LABEL_31:
  printk(&unk_3E4584, "ipa3_cfg_ep_hdr_ext");
  v20 = ipa3_ctx;
  if ( !ipa3_ctx )
    goto LABEL_36;

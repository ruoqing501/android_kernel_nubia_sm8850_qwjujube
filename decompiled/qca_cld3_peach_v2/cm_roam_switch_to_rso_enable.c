__int64 __fastcall cm_roam_switch_to_rso_enable(__int64 a1, unsigned int a2, unsigned int a3)
{
  __int64 v6; // x21
  ...
  else
  {
    ...
    if ( (supplicant_disabled_roaming & 1) != 0 )
    {
      qdf_trace_msg(
        ...
        "cm_roam_switch_to_rso_enable",
        (unsigned __int8)a2);
      result = cm_roam_state_change(a1, a2, 3u, 0x27u, 0, 0);
    }
    else
    {
      result = 0;

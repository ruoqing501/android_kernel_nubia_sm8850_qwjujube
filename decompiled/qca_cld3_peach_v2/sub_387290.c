void __usercall sub_387290(atomic_uint *a1@<X8>)
{
  unsigned int v1; // w11
  unsigned int v2; // w12
  unsigned int v3; // w20
  ...
  atomic_fetch_add_explicit(StatusReg, _ReadStatusReg(TPIDR_EL2), memory_order_relaxed);
  v11 = (atomic_uint *)_ReadStatusReg(TPIDR_EL2);
  atomic_fetch_add_explicit(a1, v8, memory_order_relaxed);
  atomic_fetch_add_explicit(v11, _ReadStatusReg(TPIDR_EL2), memory_order_relaxed);
  atomic_fetch_add_explicit(v11, _ReadStatusReg(TPIDR_EL2), memory_order_relaxed);

__int64 __fastcall wlan_hdd_update_scan_ies(__int64 result, unsigned __int8 *a2, unsigned __int16 *a3)
{
  ...
  if ( *(_WORD *)(result + 2064) )
  {
    ...
    if ( v4 )
    {
      result = (__int64)wlan_get_vendor_ie_ptr_from_oui(&unk_90BDBA, 4u, a2, *a3);
      if ( v3 >= 2 )
      {
        v15 = result;
        do
        {
          ...
          else
          {
            if ( (_DWORD)v18 != 221 )
              goto LABEL_6;
            if ( v15 || (unsigned int)qdf_mem_cmp(v4 + 2, &unk_90BDBA, 4u) )
            {
              result = qdf_mem_cmp(v4 + 2, &unk_8A74CD, 4u);
              if ( (_DWORD)result )
                goto LABEL_6;
            }
          }

__int64 __fastcall pld_srng_disable_irq_sync(__int64 a1, unsigned int a2)
{
  __int64 v3; // x20
  __int64 v5; // x0
  __int64 v6; // x22
  __int64 result; // x0
  __int64 v8; // x3
  unsigned int v9; // w8
  int v10; // w8

  v3 = pld_ctx;
  if ( a1 && pld_ctx )
  {
    v5 = raw_spin_lock_irqsave(pld_ctx + 8);
    v6 = v3 + 16;
    do
    {
      v6 = *(_QWORD *)v6;
      if ( v6 == v3 + 16 )
      {
        raw_spin_unlock_irqrestore(v3 + 8, v5);
        return printk(&unk_8EB76B, "pld_srng_disable_irq_sync", 2040, v8);
      }
    }
    while ( *(_QWORD *)(v6 - 16) != a1 );
    result = raw_spin_unlock_irqrestore(v3 + 8, v5);
    if ( v6 != 16 )
    {
      v9 = *(_DWORD *)(v6 + 16);
      if ( v9 <= 7 )
      {
        v10 = 1 << v9;
        if ( (v10 & 0xB6) != 0 )
          return result;
        if ( (v10 & 0x41) != 0 )
          return disable_irq(a2);
      }
    }
  }
  else
  {
    printk(&unk_99F123, "pld_get_dev_node", 198, a1);
  }
  return printk(&unk_8EB76B, "pld_srng_disable_irq_sync", 2040, v8);
}

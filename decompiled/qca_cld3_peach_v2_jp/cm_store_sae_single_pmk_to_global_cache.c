__int64 __fastcall cm_store_sae_single_pmk_to_global_cache(int a1, __int64 a2, __int64 a3)
{
  unsigned int v6; // w22
  int param; // w20
  __int64 result; // x0
  __int64 v9; // x20
  double psk_pmk; // d0
  double v11; // d1
  double v12; // d2
  double v13; // d3
  double v14; // d4
  double v15; // d5
  double v16; // d6
  double v17; // d7
  __int64 pmksa; // x0
  double v19; // d0
  double v20; // d1
  double v21; // d2
  double v22; // d3
  double v23; // d4
  double v24; // d5
  double v25; // d6
  double v26; // d7
  double v27; // d0
  double v28; // d1
  double v29; // d2
  double v30; // d3
  double v31; // d4
  double v32; // d5
  double v33; // d6
  double v34; // d7
  __int64 v35; // [xsp+0h] [xbp-30h] BYREF
  __int64 v36; // [xsp+8h] [xbp-28h]
  __int64 v37; // [xsp+10h] [xbp-20h]
  __int64 v38; // [xsp+18h] [xbp-18h]
  int v39; // [xsp+20h] [xbp-10h] BYREF
  __int16 v40; // [xsp+24h] [xbp-Ch]
  __int64 v41; // [xsp+28h] [xbp-8h]

  v41 = *(_QWORD *)(_ReadStatusReg(SP_EL0) + 1808);
  v6 = *(unsigned __int8 *)(a3 + 104);
  v37 = 0;
  v38 = 0;
  v35 = 0;
  v36 = 0;
  v40 = 0;
  v39 = 0;
  param = wlan_crypto_get_param(a3, 7u);
  result = wlan_cm_roam_cfg_get_value(a1, v6, 25, (int)&v35);
  if ( (unsigned __int8)v36 == 1 && (param & 0x8000400) != 0 )
  {
    wlan_vdev_get_bss_peer_mac_for_pmksa();
    wlan_crypto_set_sae_single_pmk_bss_cap(a3, (__int64)&v39, 1);
    result = _qdf_mem_malloc(0x50u, "cm_store_sae_single_pmk_to_global_cache", 6118);
    if ( result )
    {
      v9 = result;
      psk_pmk = wlan_cm_get_psk_pmk(a2, v6, result, result + 64);
      pmksa = wlan_crypto_get_pmksa(a3, (__int64)&v39, psk_pmk, v11, v12, v13, v14, v15, v16, v17);
      if ( pmksa )
      {
        *(_WORD *)(v9 + 66) = *(_DWORD *)(pmksa + 124) * (unsigned int)*(unsigned __int8 *)(pmksa + 128) / 0x64;
        *(_QWORD *)(v9 + 72) = *(_QWORD *)(pmksa + 136);
        qdf_trace_msg(
          0x68u,
          8u,
          "%s: spmk_ts:%ld spmk_timeout_prd:%d secs",
          v19,
          v20,
          v21,
          v22,
          v23,
          v24,
          v25,
          v26,
          "cm_store_sae_single_pmk_to_global_cache",
          v35);
      }
      else
      {
        qdf_trace_msg(
          0x68u,
          8u,
          "%s: PMK entry not found for bss:%02x:%02x:%02x:**:**:%02x",
          v19,
          v20,
          v21,
          v22,
          v23,
          v24,
          v25,
          v26,
          "cm_store_sae_single_pmk_to_global_cache",
          (unsigned __int8)v39,
          BYTE1(v39),
          BYTE2(v39),
          HIBYTE(v40),
          v35);
      }
      wlan_mlme_update_sae_single_pmk(a3, v9, v27, v28, v29, v30, v31, v32, v33, v34);
      qdf_mem_set((void *)v9, 0x50u, 0);
      result = _qdf_mem_free(v9);
    }
  }
  _ReadStatusReg(SP_EL0);
  return result;
}

__int64 __fastcall wlan_cm_set_ese_assoc(__int64 a1, unsigned __int8 a2, char a3)
{
  _QWORD *vdev_by_id_from_pdev; // x0
  double v5; // d0
  double v6; // d1
  double v7; // d2
  double v8; // d3
  double v9; // d4
  double v10; // d5
  double v11; // d6
  double v12; // d7
  __int64 ext_hdl_fl; // x8
  _QWORD *v14; // x20

  vdev_by_id_from_pdev = wlan_objmgr_get_vdev_by_id_from_pdev(a1, a2, 0x4Du);
  if ( !vdev_by_id_from_pdev )
    return qdf_trace_msg(
             0x68u,
             2u,
             "%s: vdev object is NULL",
             v5,
             v6,
             v7,
             v8,
             v9,
             v10,
             v11,
             v12,
             "wlan_cm_set_ese_assoc");
  ext_hdl_fl = *((unsigned int *)vdev_by_id_from_pdev + 4);
  if ( (ext_hdl_fl & 0xFFFFFFFD) == 0 )
  {
    v14 = vdev_by_id_from_pdev;
    ext_hdl_fl = cm_get_ext_hdl_fl(
                   (__int64)vdev_by_id_from_pdev,
                   (__int64)"wlan_cm_set_ese_assoc",
                   0x46Eu,
                   v5,
                   v6,
                   v7,
                   v8,
                   v9,
                   v10,
                   v11,
                   v12);
    vdev_by_id_from_pdev = v14;
    wlan_cm_set_ese_assoc_rso_cfg = ext_hdl_fl;
    if ( ext_hdl_fl )
      *(_BYTE *)(ext_hdl_fl + 678) = a3 & 1;
  }
  return wlan_objmgr_vdev_release_ref(
           (__int64)vdev_by_id_from_pdev,
           0x4Du,
           (unsigned int *)ext_hdl_fl,
           v5,
           v6,
           v7,
           v8,
           v9,
           v10,
           v11,
           v12);
}

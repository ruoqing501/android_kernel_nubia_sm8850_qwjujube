...
{
	static DEFINE_RATELIMIT_STATE(cfi_rs, DEFAULT_RATELIMIT_INTERVAL, DEFAULT_RATELIMIT_BURST);
	bool should_report = __ratelimit(&cfi_rs);

	if (should_report) {
		if (target)
			pr_warn("CFI failure at %pS (target: %pS; expected type: 0x%08x) - WARNING ONLY\n",
			       (void *)addr, (void *)*target, type);
		else
			pr_warn("CFI failure at %pS (no target information) - WARNING ONLY\n",
			       (void *)addr);

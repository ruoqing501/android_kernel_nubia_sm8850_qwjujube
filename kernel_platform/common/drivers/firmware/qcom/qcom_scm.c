 ...
 */
static int qcom_scm_qseecom_call_local(const struct qcom_scm_desc *desc,
				 struct qcom_scm_qseecom_resp *res)
{
	int status;
...
static int qcom_scm_qseecom_get_version(u32 *version)
{
	...
	ret = qcom_scm_qseecom_call_local(&desc, &res);
	if (ret)
		return ret;
...
int qcom_scm_qseecom_app_get_id(const char *app_name, u32 *app_id)
{
	...
	status = qcom_scm_qseecom_call_local(&desc, &res);
...
int qcom_scm_qseecom_call(u32 smc_id, struct qseecom_scm_desc *desc, bool atomic)
{
	...
	if (atomic)
		status = qcom_scm_call_atomic(__scm ? __scm->dev : NULL, &qcom_desc, &res);
	else
		status = qcom_scm_call(__scm ? __scm->dev : NULL, &qcom_desc, &res);
...
/* ZTE RedMagic SCM Parity Bridge Functions */
